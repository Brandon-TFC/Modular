...
def index(request):
    ...
    # Render recibe la peticion request||En el HTML devulve la ruta||El dicc es para el contexto que se muestran en el html
...
def login(request):
    ...
    if request.method == 'POST':
        ...
        if usuarios:
            lg(request, usuarios)
            messages.success(request, f'Hola {usuarios.username}, Bienvenido.')
...
def registro(request):
    ...
    if request.method == 'POST' and form.is_valid():
        usuario = form.save()
        if usuario: #Si el usario se registro le daremos un mensaje
            lg(request, usuario)
